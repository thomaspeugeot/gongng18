...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	// log.Println("Stack gongng18/go, onWebSocketRequestForBackRepoContent")
	...
	log.Printf("Stack gongng18/go: stack path: '%s', new ws index %d",
	...
	if backRepo == nil {
		log.Panic("Stack gongng18/go, Unkown stack", stackPath)
	}
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("gongng18/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		log.Println("gongng18/go:\n",
			"client no longer receiver web socket message, assuming it is no longer alive, closing websocket handler")
		fmt.Println(err)
		return
	} else {
		log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "gongng18/go: 1st sent backRepoData of stack:", stackPath, "index", index)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "gongng18/go: sent backRepoData of stack:", stackPath, "index", index)
				}
			}
		}
	}
